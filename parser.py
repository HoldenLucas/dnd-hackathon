...
# Speaker names from Naddpod, mapped to a unique peer ID for Honcho.
...
def parse_transcript(transcript_text):
    """
    Parses the transcript text into a list of messages with speaker and content.
    Assumes each line is formatted as 'Speaker Name: Dialogue'.
    """
    messages = []
    ...
    for line in lines:
        ...
        if match:
            ...
            # Find the peer_id for the speaker name
            peer_id = SPEAKERS.get(speaker_name)
            if peer_id:
                messages.append({
                    "peer_id": peer_id,
                    "content": content
                ...
                print(f"Warning: Speaker '{speaker_name}' not found in the SPEAKERS mapping. Skipping line.")
    return messages

if __name__ == "__main__":
    # Example transcript content. Replace this with the path to your transcript file.
    # For a real file, you would use:
...
"""
    ...
    # 1. Parse the local transcript data
    parsed_messages = parse_transcript(transcript_content)
    
    # 2. Print the formatted messages to the console
